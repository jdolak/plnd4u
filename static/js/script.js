...
function addCourse() {
    ...
    const overlay = document.getElementById(
        "unlisted-course-overlay-container"
    );
    $.ajax({
        url: "/classes",
        type: "POST",
...
function showModal(courseCode, corequisites, prerequisites) {
    ...
    if (corequisites != null) {
        ...
        if (courseCode in corequisites) {
            corequisites[courseCode].forEach(function (course) {
                if (!text) {
                    text = text + "Corequisites: " + course;
                } else {
                    text = text + ", " + course;
                }
            });
    ...
    var startPrerequisites = true;

    console.log(courseCode, prerequisites);

    if (prerequisites != null) {
        if (courseCode in prerequisites) {
            prerequisites[courseCode].forEach(function (course) {
                if (startPrerequisites) {
                    text = text + "Prerequisites: " + course;
                    startPrerequisites = false;
                } else {
                    text = text + ", " + course;
                }
            });
...
function createModal() {
    ...
    modalHeader.className = "modal-container-header";
    modalHeader.textContent = `Missing Requisites`;
...
function showInPlan(enrollments, semester, corequisites, prerequisites) {
    ...
    enrollments.forEach(function (enrollment) {
        ...
        console.log(prerequisites, courseCode);
        ...
        removeButton.addEventListener("click", function () {
            ...
            const courseCode = this.closest(
                ".plan-card-single-course"
            ).querySelector(".plan-card-single-course-code").textContent;
            const courseName = this.closest(
                ".plan-card-single-course"
            ).querySelector(".plan-card-single-course-name").textContent;
...
function searchClasses() {
    ...
    const fallSemesterCheckbox = document.querySelector(
        'input[name="fall-semester"]'
    );
    const springSemesterCheckbox = document.querySelector(
        'input[name="spring-semester"]'
    );
    ...
    const majorElectiveCheckbox = document.querySelector(
        'input[name="major-elective"]'
    );
...
function courseDel(courseCode, courseName, semester) {
    ...
    const planCards = planCardContainer.getElementsByClassName(
        "plan-card-single-course"
    );
    for (const planCard of planCards) {
        const classCode = planCard.querySelector(
            ".plan-card-single-course-code"
        ).textContent;
        const className = planCard.querySelector(
            ".plan-card-single-course-name"
        ).textContent;
...
function courseOverviewOverlay(
    ...
    const overlay = document.getElementById(
        "course-overview-overlay-container"
    );
    const content = document.getElementById("course-overview-overlay-content");
    const courseCodeContent = document.getElementById(
        "course-overview-course-code"
    );
    const courseNameContent = document.getElementById(
    ...
    var joinedPrereqs = prereqs.map(
        (innerArray) => "(" + innerArray[0].split(",").join(" or ") + ")"
    );
    coursePrereqs.textContent =
        "Prerequisite(s): " + joinedPrereqs.join(" AND ");
...
function degreeCompletionOverlayOn() {
    ...
    const coreRequirements = document.getElementById(
        "degree-details-core-reqs"
    );
    const requiredCourses = document.getElementById(
        "degree-details-major-reqs"
    );
    ...
    $.ajax({
        ...
        success: function (response) {
            if (response.core_requirements.split(",").join("") != "None") {
                coreRequirements.innerHTML =
                    'Missing: <span style="color: black;">' +
                    response.core_requirements.split(",").join(", ") +
                    "</span>";
                coreRequirements.getElementsByTagName("span")[0].style.color =
                    "red";
            } else {
            ...
            if (response.required_courses.split(",").join("") != "None") {
                requiredCourses.innerHTML =
                    'Missing: <span style="color: black;">' +
                    response.required_courses.split(",").join(", ") +
                    "</span>";
                requiredCourses.getElementsByTagName("span")[0].style.color =
                    "red";
            } else {
...
if (onClassesPage()) {
    document.addEventListener("DOMContentLoaded", function () {
        ...
        const overlay = document.getElementById(
            "semester-filter-overlay-container"
        );

        button.addEventListener("click", function () {
            const buttonRect = document
                .getElementById("semester-filter-container")
    ...
    document.addEventListener("DOMContentLoaded", function () {
        ...
        const overlay = document.getElementById(
            "level-filter-overlay-container"
        );

        button.addEventListener("click", function () {
            const buttonRect = document
                .getElementById("level-filter-container")
                .getBoundingClientRect();
            const containerTop = buttonRect.bottom + window.scrollY;
            const containerLeft = buttonRect.left + window.scrollX;
    ...
    document.addEventListener("DOMContentLoaded", function () {
        ...
        button.addEventListener("click", function () {
            const buttonRect = document
                .getElementById("req-filter-container")
                .getBoundingClientRect();
            const containerTop = buttonRect.bottom + window.scrollY;
            const containerLeft = buttonRect.left + window.scrollX;
