...
function showInPlan(enrollments, semester, prerequisites, corequisites) {
  ...
  enrollments.forEach(function (enrollment) {
    ...
    // } else if (courseCode in prerequisites) {
    ...
    if (corequisites || prerequisites) {
      courseCard.addEventListener("click", function () {
        showModal(courseCode, prerequisites, corequisites);
      });
    }

    // if (courseCode in corequisites || courseCode in prerequisites) {
    //   courseCard.classList.add(
    //     courseCode in corequisites
    //       ? "plan-card-single-course-red"
    //       : "plan-card-single-course-yellow"
    //   );
    //   courseCard.style.cursor = "pointer"; // Change cursor on hover
    ...
    removeButton.addEventListener("click", function () {
      ...
      const courseCode = this.closest(".plan-card-single-course").querySelector(
        ".plan-card-single-course-code"
      ).textContent;
      const courseName = this.closest(".plan-card-single-course").querySelector(
...
function courseOverviewOverlay(
  ...
  var joinedPrereqs = prereqs.map((innerArray) =>
    innerArray[0].split(",").join(", ")
  );
